"""
V14 Cross-Asset Backtest — Building on V13.13, targeting 10%+/mo per asset.

Tests V14 on all 4 assets (BTC, ETH, SOL, LINK) with:
- Per-asset results
- Regime breakdown
- Long vs short breakdown
- OOS validation (60/40 split)
- Comparison to V13 baseline
...
class V14Engine(BacktestEngine):
    ...
    def run_with_kelly_feedback(self, data, strategy, name="unnamed", cross_momentum=None):
        ...
        for i in range(20, len(data)):
            ...
            # V14: Update cross-asset momentum and equity for strategy
...
def main():
    ...
    print("=" * 90)
    print("  V14 CROSS-ASSET BACKTEST — Target: ALL assets 10%+/mo")
    print(f"  {total_bars} bars/asset | {date_range}")
    print(f"  Target: ALL 4 assets 10%+/mo, max DD <25%")
    print("=" * 90)

    # =====================================================
    # 1. Per-asset V14 results with Kelly feedback
    # =====================================================
    print("\n" + "=" * 90)
    print("  [1] V14 PER-ASSET RESULTS (Kelly feedback enabled)")
    print("=" * 90)
    ...
    for sym in ASSETS:
        ...
        ema_fast = closes.ewm(span=192, adjust=False).mean()  # ~8d
        ema_slow = closes.ewm(span=504, adjust=False).mean()  # ~21d
        asset_trends[sym] = (ema_fast > ema_slow).astype(int)  # 1=bullish, 0=bearish

    # Per-bar momentum: fraction of assets trending up, mapped to -1..+1
    ...
    for idx in common_idx:
        ...
        cross_momentum[idx] = (bull_count / len(ASSETS)) * 2 - 1  # -1 to +1

    for sym in ASSETS:
        data = datasets[sym]
        max_risk = ASSET_MAX_RISK.get(sym, 8.0)
        engine = V14Engine(initial_capital=1000, fee_pct=0.045, max_risk_pct=max_risk)
        strat = SqueezeV14(btc_data=btc_data, asset_name=sym)
        if sym != "BTC":
            strat.set_btc_data(btc_data)

        result, kelly_hist = engine.run_with_kelly_feedback(data, strat, f"V14 {sym}",
                                                            cross_momentum=cross_momentum)
        monthly = mo(result, data)
        v14_results[sym] = (result, monthly)
        v14_kelly[sym] = kelly_hist
        v14_strats[sym] = strat
    ...
    # Summary
    print(f"\n  {'─' * 50}")
    all_monthly = [v14_results[s][1] for s in ASSETS]
    ...
    print(f"  Per-asset: {', '.join(f'{s}:{v14_results[s][1]:+.2f}' for s in ASSETS)}")
    ...
    for sym in ASSETS:
        strat = v14_strats[sym]
        result = v14_results[sym][0]
    ...
    # =====================================================
    # 3. V13 vs V14 Comparison
    # =====================================================
    print("\n" + "=" * 90)
    print("  [3] V13 vs V14 COMPARISON")
    print("=" * 90)

    print(f"\n  {'Asset':<6} {'V13 %/mo':>10} {'V14 %/mo':>10} {'Delta':>10} {'V14 DD':>8}")
    for sym in ASSETS:
        data = datasets[sym]
        # Run V13 baseline
        max_risk = ASSET_MAX_RISK.get(sym, 8.0)
        engine13 = V14Engine(initial_capital=1000, fee_pct=0.045, max_risk_pct=max_risk)
        strat13 = SqueezeV13(btc_data=btc_data, asset_name=sym)
        if sym != "BTC":
            strat13.set_btc_data(btc_data)
        r13, _ = engine13.run_with_kelly_feedback(data, strat13, f"V13 {sym}")
        m13 = mo(r13, data)
        m14 = v14_results[sym][1]
        dd14 = v14_results[sym][0].max_drawdown_pct
        delta = m14 - m13
        better = "+" if delta > 0 else ""
        print(f"  {sym:<6} {m13:+10.2f} {m14:+10.2f} {better}{delta:9.2f} {dd14:7.1f}%")
    ...
    for sym in ASSETS:
        ...
        engine_tr = V14Engine(initial_capital=1000, fee_pct=0.045, max_risk_pct=ASSET_MAX_RISK.get(sym, 8.0))
        strat_tr = SqueezeV14(btc_data=btc_data, asset_name=sym)
        if sym != "BTC":
        ...
        engine_te = V14Engine(initial_capital=1000, fee_pct=0.045, max_risk_pct=ASSET_MAX_RISK.get(sym, 8.0))
        strat_te = SqueezeV14(btc_data=btc_data, asset_name=sym)
        if sym != "BTC":
    ...
    max_dd = max(v14_results[s][0].max_drawdown_pct for s in ASSETS)
    print(f"  Max DD across assets: {max_dd:.1f}%")

    targets_met = all_above_10 and max_dd < 25.0
    print(f"\n  TARGET MET: {'YES' if targets_met else 'NO'}")
    if not all_above_10:
        below = [s for s in ASSETS if v14_results[s][1] < 10.0]
        print(f"  Below 10%: {', '.join(f'{s}:{v14_results[s][1]:+.2f}' for s in below)}")

    elapsed = time.time() - t_start
    print(f"\n  Runtime: {elapsed:.1f}s")

    return v14_results, all_above_10, avg_monthly
